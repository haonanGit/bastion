...
WSClient::WSClient(const std::string_view &addr,
  ...
  client_.set_tls_init_handler([](websocketpp::connection_hdl) {
    ...
    // // 启用详细的 OpenSSL 调试信息
    // SSL_CTX_set_info_callback(ctx->native_handle(), [](const SSL* ssl, int
    // type, int val) {
  ...
  client_.set_message_handler(
      [this](const websocketpp::connection_hdl hdl,
             app_tls_client::message_ptr msg) { onMessage(hdl, msg); });
  client_.set_fail_handler(
      [this](const websocketpp::connection_hdl hdl) { onFail(hdl); });
}
...
void WSClient::stop() { client_.stop(); }
