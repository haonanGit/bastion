...
private:
    ...
    // DPDK-related member variables
    uint16_t            port_id;
    struct rte_mempool* mbuf_pool;

private:
    void dpdk_init();
    void dpdk_send(const std::string& msg);

public:
    WSClient() = default;
    ~WSClient() = default;
    WSClient(const std::string_view& addr, const std::string_view& subs_msg, const std::string_view& cancel_subs_msg);
    virtual void start();
    virtual void stop();
