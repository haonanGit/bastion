...
char *Base64Encode(const unsigned char *input,
...
long long convertToTimestamp(const std::string &dateTimeStr, TimeUnit unit) {
  ...
  // 获取毫秒、微秒或纳秒部分
  std::string subsecondStr =
      dateTimeStr.substr(20); // 假设子秒部分在第20个字符后
  int subsecond = 0;
...
std::string hmac_sha256HexString(const std::string &secret,
  ...
  auto hash = HMAC(EVP_sha256(), secret.c_str(), secret.length(),
                   (unsigned char *)sign.c_str(), sign.length(), NULL, NULL);
  return toHexString(hash, SHA256_DIGEST_LENGTH);
}
