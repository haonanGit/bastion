...
std::unordered_map<std::string, std::string> msg_type_dict = {{"0", "MsgType_Heartbeat"},
...
std::unordered_map<std::string, std::string> session_reject_reason_dict = {
    {std::to_string(FIX::SessionRejectReason_INVALID_TAG_NUMBER), "Invalid tag number"},
    {std::to_string(FIX::SessionRejectReason_REQUIRED_TAG_MISSING), "Required tag missing"},
    {std::to_string(FIX::SessionRejectReason_TAG_NOT_DEFINED_FOR_THIS_MESSAGE_TYPE), "Tag not defined for this message type"},
    {std::to_string(FIX::SessionRejectReason_UNDEFINED_TAG), "Undefined Tag"},
    {std::to_string(FIX::SessionRejectReason_TAG_SPECIFIED_WITHOUT_A_VALUE), "Tag specified without a value"},
    {std::to_string(FIX::SessionRejectReason_VALUE_IS_INCORRECT), "Value is incorrect (out of range) for this tag"},
    {std::to_string(FIX::SessionRejectReason_INCORRECT_DATA_FORMAT_FOR_VALUE), "Incorrect data format for value"},
    {std::to_string(FIX::SessionRejectReason_DECRYPTION_PROBLEM), "Decryption problem"},
    {std::to_string(FIX::SessionRejectReason_SIGNATURE_PROBLEM), "Signature problem"},
    {std::to_string(FIX::SessionRejectReason_COMPID_PROBLEM), "CompID problem"},
    {std::to_string(FIX::SessionRejectReason_SENDINGTIME_ACCURACY_PROBLEM), "SendingTime accuracy problem"},
    {std::to_string(FIX::SessionRejectReason_INVALID_MSGTYPE), "Invalid MsgType"},
    {std::to_string(FIX::SessionRejectReason_XML_VALIDATION_ERROR), "XML validation error"},
    {std::to_string(FIX::SessionRejectReason_TAG_APPEARS_MORE_THAN_ONCE), "Tag appears more than once"},
    {std::to_string(FIX::SessionRejectReason_TAG_SPECIFIED_OUT_OF_REQUIRED_ORDER), "Tag specified out of required order"},
    {std::to_string(FIX::SessionRejectReason_REPEATING_GROUP_FIELDS_OUT_OF_ORDER), "Repeating group fields out of order"},
    {std::to_string(FIX::SessionRejectReason_INCORRECT_NUMINGROUP_COUNT_FOR_REPEATING_GROUP), "Incorrect NumInGroup count for repeating group"},
    {std::to_string(FIX::SessionRejectReason_NON_DATA_VALUE_INCLUDES_FIELD_DELIMITER), "Non-data value includes field delimiter"},
    {std::to_string(FIX::SessionRejectReason_INVALID_UNSUPPORTED_APPLICATION_VERSION), "Invalid/unsupported application version"},
...
std::unordered_map<std::string, std::string> order_status_dict = {{std::string(1, FIX::OrdStatus_NEW), "New"},
                                                                  ...
                                                                  {std::string(1, FIX::OrdStatus_ACCEPTED_FOR_BIDDING), "Accepted for Bidding"},
                                                                  {std::string(1, FIX::OrdStatus_PENDING_REPLACE), "Pending Replace"}};
/*------------------------------------------------raw string result--------------------------------------------------------*/

std::unordered_map<std::string, std::string> raw_str_dict = {{"7", "BeginSeqNo"},
                                                             {"8", "BeginString"},
                                                             {"9", "BodyLength"},
                                                             {"10", "CheckSum"},
                                                             {"16", "EndSeqNo"},
