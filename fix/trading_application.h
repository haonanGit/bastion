...
std::string generateRawData() {
    ...
    // std::string base64_nonce = common::Base64Encode(reinterpret_cast<const unsigned char*>(test_nonce), strlen(test_nonce));
...
std::string generatePassword(const std::string& raw_data) {
    ...
    // std::cout << "base_signature_string:" << base_signature_string << std::endl;
    std::string sha256_str = common::sha256RawString((base_signature_string));
    std::string password = common::Base64Encode(reinterpret_cast<const unsigned char*>(sha256_str.c_str()), strlen(sha256_str.c_str()));
...
public:
    ...
    void sendSequenceReset(const FIX::SessionID& session_id, const int& seq = 1) {
        ...
        auto                 expected_seq_num = FIX::Session::lookupSession(session_id)->getExpectedTargetNum();
        ...
        try {
            ...
            std::cerr << "Sequence Reset: Session not found: " << e.what() << std::endl;
        } catch (...) {
