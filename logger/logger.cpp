...
void Logger::init(const Options& options) {
    ...
    auto sink = std::make_shared<spdlog::sinks::rotating_file_sink_mt>(file_path, options.max_size, options.max_files, true);
    if (options.is_async) {
        thread_pool_ = std::make_shared<spdlog::details::thread_pool>(1024 * 5, 1);
        async_file_logger_ = std::make_shared<spdlog::async_logger>(options.name, sink, thread_pool_, spdlog::async_overflow_policy::overrun_oldest);
